...
class QOnlineRetail:
    ...
    def train_data (self):
        ...
        x_train = np.zeros ((self.num_queries, train_length, 1 + self.num_queries))
        for q in range (self.num_queries):
            
            for i in range (int(self.training[0] * len (self.x_data)), int(self.training[1] * len (self.x_data))):
                x_train[q][i-int (self.training[0]*len (self.x_data))][0] = self.x_data[i][q]
                x_train[q][i-int (self.training[0]*len (self.x_data))][q+1] = 1
        ...
        for q in range (self.num_queries):
            for i in range (int(self.training[0]*len (self.y_data)), int (self.training[1]*len (self.y_data))):
                y_train [q][i-int (self.training[0]*len (self.y_data))][self.y_data[i][q]] = 1
        #y_train = []
        ...
        x_val = np.zeros ((self.num_queries, int(self.validation[1] * len (self.x_data))-int(self.validation[0] * len (self.x_data)), 1 + self.num_queries))
        for q in range (self.num_queries):
            
            for i in range (int(self.validation[0] * len (self.x_data)), int(self.validation[1] * len (self.x_data))):
                x_val[q][i-int (self.validation[0]*len (self.x_data))][0] = self.x_data[i][q]
                x_val[q][i-int (self.validation[0]*len (self.x_data))][q+1] = 1
        
        #encoding using one-hot method
        y_val = np.zeros ((self.num_queries, int(self.validation[1]*len (self.y_data)) - int (self.validation[0]*len (self.y_data)), 2))
        for q in range (self.num_queries):
            for i in range (int(self.validation[0]*len (self.y_data)), int (self.validation[1]*len (self.y_data))):
                y_val [q][i-int(self.validation[0]*len (self.y_data))][self.y_data[i][q]] = 1
        #y_val = []
...
def simple_model (input_shape, num_output_categories):
    ...
    model.add(RNN(HIDDEN_SIZE, input_shape=(None, input_shape[1]),return_sequences=True))
...
def test_model (model, input, output, verbose=False):
    ...
    for q in range (len (input)):
        ...
        err = l2_error (real[q][8:], prediction[q][8:])
        total_l2_error += err
        err = l1_error (real[q][8:], prediction[q][8:])
        total_l1_error += err
        if verbose:
            ...
            print ("")
    total_l2_error = total_l2_error / (len (real[0])-8)
    print ("Average Error L2: " + str (total_l2_error))
    
    rmsd = (total_l2_error / len (real))**0.5
    print ("RMSD: " + str(rmsd))
    
    total_l1_error = total_l1_error / (len (real[0])-8)
